...
DROP TRIGGER IF EXISTS FilmSphere.RestrizioneAudioInsert;
DELIMITER $$
CREATE TRIGGER FilmSphere.RestrizioneAudioInsert BEFORE INSERT ON Restrizione
FOR EACH ROW
BEGIN
...
DROP TRIGGER IF EXISTS FilmSphere.RestrizioneAudioUpdate;
DELIMITER $$
CREATE TRIGGER FilmSphere.RestrizioneAudioUpdate BEFORE UPDATE ON Restrizione
FOR EACH ROW
BEGIN
    DECLARE _corretto INTEGER DEFAULT 0;
    SET _corretto = (SELECT ID
...
-- Verifica che in Restrizione ci siano solo Formati esistenti nel DB
DROP TRIGGER IF EXISTS FilmSphere.RestrizioneVideoInsert;
DELIMITER $$
CREATE TRIGGER FilmSphere.RestrizioneVideoInsert BEFORE INSERT ON Restrizione
FOR EACH ROW
BEGIN
    DECLARE _corretto INTEGER DEFAULT 0;
    SET _corretto = (SELECT ID
...
DROP TRIGGER IF EXISTS FilmSphere.RestrizioneVideoUpdate;
DELIMITER $$
CREATE TRIGGER FilmSphere.RestrizioneVideoUpdate BEFORE UPDATE ON Restrizione
FOR EACH ROW
BEGIN
    DECLARE _corretto INTEGER DEFAULT 0;
    SET _corretto = (SELECT ID
...
CREATE PROCEDURE FilmSphere.ADD_EDGE_SERVER(IN IpServer VARCHAR(15), IN Latitudine FLOAT, IN Longitudine FLOAT, OUT _check BOOL)
BEGIN
	DECLARE finito INTEGER DEFAULT 0;
    DECLARE fetchServer VARCHAR(15) DEFAULT '';
    DECLARE fetchLat FLOAT DEFAULT 0;
